...
def parse(opt_path):
    ...
    if torch.cuda.is_available():
        ...
        os.environ['CUDA_VISIBLE_DEVICES'] = str(1)#gpu_list
    ...
    #config_str = opt['dir']
    exp_path = opt['dir']# os.path.join(os.getcwd(), 'experiments', config_str)
...
def parse_imagetest(opt_path):
    ...
    config_str = '%s_in%df%d_x%d'%(network_opt['which_model'].upper(), network_opt['in_channels'],
                                                        network_opt['num_features'], opt['scale'])
    exp_path = opt['dir']#exp_path = os.path.join(os.getcwd(), 'experiments', config_str)
