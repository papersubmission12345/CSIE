...
def get_image_paths_from_txt(dataroot,need_HR):
    LRpaths= []
    HRpaths = []
    QS = [] #quality score
    lr_or_paths = np.loadtxt(dataroot, dtype=np.str)
   
    for index in range(len(lr_or_paths)):
        if need_HR == 0: #LR only
            cvtostring0 = str(lr_or_paths[index])
            LRpaths.append(cvtostring0)
        else: ##Lr, HR and score
            cvtostring0 = str(lr_or_paths[index][0])
            cvtostring1 = str(lr_or_paths[index][1])
            cvtostring2 = str(lr_or_paths[index][2])
            cvtodouble = np.double(cvtostring2)
            
            LRpaths.append(cvtostring0)
            HRpaths.append(cvtostring1)
            QS.append(cvtodouble)
            
    return LRpaths,HRpaths,QS
...
def read_img_fromtxt(path, data_type):
    # read image by misc or from .npy
    # return: Numpy float32, HWC, RGB, [0,255]
    #
    #print('read_img_fromtxt')
    if data_type == 'img':
        img = imageio.imread(path)#, pilmode='RGB')
    elif data_type.find('npy') >= 0:
        img = np.load(path)
