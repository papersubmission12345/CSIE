...
class SRSolver(BaseSolver):
    def __init__(self, opt):
        ...
        self.HR = self.Tensor()
        self.SR = None
        self.LR1 = self.Tensor()
        self.LR2 = self.Tensor()
        self.QS = self.Tensor()#label
        ...
        self.model = self.model.cuda()#to(torch.device('cuda:0'))
        if self.is_train:
            self.model.train()
    ...
    def feed_imgs(self, LR, SR, need_HR=True):
        input1 = LR #get the input of quality enhancement
        input2 = SR #get the output of quality enhancement
        self.LR1.resize_(input1.size()).copy_(input1)
        self.LR2.resize_(input2.size()).copy_(input2)
    ...
    def test_scorenet(self):
        self.model.eval()
        with torch.no_grad():
            forward_func = self._overlap_crop_forward_scorenet if self.use_chop else self.model.forward
            if self.self_ensemble and not self.is_train:
                predQS = self._forward_x8(self.LR, forward_func)
                print('self_ensemble')
            else:
                predQS = forward_func(self.LR1, self.LR2)
            predQS = forward_func(self.LR1, self.LR2)

            if isinstance(predQS, list):
                self.predQS = predQS[-1]
            else:
                self.predQS = predQS

        self.model.train()
        if self.is_train:
            #loss = criterion_pix of output and SSIM of Lr1 (input of Quality enhance) and the real HR
            QS = self.QS
            QS = QS.view(len(predQS),-1)
            loss_pix = self.criterion_pix(self.predQS, QS.cuda())
            return loss_pix.item()
    
    def get_current_visual_scorenet(self, need_np=False, need_HR=True):
        """
        return LR SR (HR) images
        """
        out_dict = OrderedDict()
        out_dict['LR1'] = self.LR1.data[0].float().cpu()
        out_dict['LR2'] = self.LR2.data[0].float().cpu()
        out_dict['predQS'] = self.predQS.data[0].float().cpu()
        if need_np:  
            out_dict['LR1'], out_dict['LR2'] = util.Tensor2np([out_dict['LR1'], out_dict['LR2']], self.opt['rgb_range'])
        if need_HR:
            out_dict['QS'] = self.QS.data[0].float().cpu()
        return out_dict
    ...
    def _overlap_crop_forward_scorenet(self, x1,x2, shave=10, min_size=100000, bic=None):
        """
        ...
        scale = self.scale
        b, c, h, w = x1.size()
        #split image into patches and forward through CNN
        size_input = 224
        count = 0
        sum_score = 0
        
        for x in range(0,h-size_input+1,40):
            for y in range(0,w-size_input+1,40):
                subim_input1 = x1[:, :, x : x+size_input, y : y+size_input]
                subim_input2 = x2[:, :, x : x+size_input, y : y+size_input]
                count = count+1
                score = self.model(subim_input1,subim_input2)
                sum_score += score

        output=sum_score/count
        return output
    ...
    def save_checkpoint(self, dir, epoch, is_best):
        """
        save checkpoint to experimental dir
        """
        filename = os.path.join(dir, 'last_ckp.pth')
