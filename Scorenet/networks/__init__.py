...
def define_net(opt):
    ...
    if which_model == 'DBPN':
        ...
        net = EDSR(in_channels=opt['in_channels'], out_channels=opt['out_channels'],
                             ...
                             upscale_factor=opt['scale'])
    elif which_model.find('SKIPRR') >= 0:
        from .SkipRR_arch import SKIPRR
        net = SKIPRR(in_channels=opt['in_channels'], out_channels=opt['out_channels'], num_features=opt['num_features'], num_steps=opt['num_steps'])
    elif which_model.find('SCORE') >= 0:
        from .scorenet_arch import SCORE
        net = SCORE(in_channels=opt['in_channels'], num_features=opt['num_features'])
